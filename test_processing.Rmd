...
## Missing data - IPW - not working

```{r}

# Geração de pesos de probabilidade inversa; atruibuir pesos para que uma pessoa "Valha" mais que a outra
# Pesos a pessoas com maior probabilidade de não estarem sendo representadas no banco = aumentar a representatividade para representar alguém que abandonou

# Criar variável de retenção (sim = quem ficou; é o inverso do missing)
...
## Selecionando variáveis demográficas (predizendo quem vai ficar e sair)
bhrc_database_ipw_mfq <- bhrc_database_ipw_mfq %>% dplyr::select(subjectid, ident, Age:Site, Child_race:SEG, School_repet:work_bin, minstlevel: Assistance_type, Income_total:abepstrat, attrition_MFQ)

bhrc_database_ipw_mfq <- bhrc_database_ipw_mfq %>% dplyr::select(-minstlevel_4cat, -SEG, -Educ_level_current, -Educ_attainment, -Child_race_2cat)

#Filter to complete data in variable with less complete baseline data
complete.cases(bhrc_database_ipw_mfq)
bhrc_database_ipw_mfq <- bhrc_database_ipw_mfq[complete.cases(bhrc_database_ipw_mfq), ]

## Descriptive (!!!!!!!!!) quem não ficou VS quem ficou no estudo
table1(~ Age + Gender + Selection + Site + Child_race + School_repet +  School_exp + School_drop + School_evasion + School_susp + Studying + Educ_years + work_bin + minstlevel + finstlevel + work + moccupation +  foccupation + Assistance_any + Assistance_type + Income_total + Conviction +  abepscore + abepstrat | attrition_MFQ, data = bhrc_database_ipw_mfq, overall=FALSE)

## Logit regression
# Usar fit para modelos de regressão
fit_ipw_mfq <- glm(attrition_MFQ ~ Age + Gender + Selection + Site + Child_race_2cat + Educ_years + minstlevel_4cat + work_bin + Income_total + Conviction +  abepscore + abepstrat, family=binomial(link='logit'), data=bhrc_database_ipw_mfq)
...
bhrc_database_ipw_mfq <- cbind(bhrc_database_ipw_mfq,ipw_mfq) %>% dplyr::select(ident, ipw_mfq, attrition_MFQ)

### testing - testar se o peso deu certo; deve-se somar todos os pesos no banco das pessoas que ficaram e ver se o resultado dá o mesmo do n inicial
bhrc_database_ipw_mfq_test<- bhrc_database_ipw_mfq %>% filter(attrition_MFQ=="Yes")
sum(bhrc_database_ipw_mfq_test$ipw_mfq)  #should be = x (complete variables at baseline)
```

```{r}

bhrc_database_ipw_mfq <- na.omit(bhrc_database_ipw_mfq)
# Pacote ipw
weights_ipwpoint <- ipwpoint(
  exposure = attrition_MFQ,
  family = "binomial",  # The treatment is binary
  link = "logit",
  denominator = ~ Age + Gender + Selection + Site + Educ_years + minstlevel_4cat + work_bin + Income_total + Conviction +  abepscore + abepstrat,
  data = bhrc_database_ipw_mfq
)
